# Generated red team — 190 model-written attacks vs the openai seller
...
**190/190 defended (100.0%)** against the `openai` seller. The defence is architectural: even if a model *proposed* an off-table offer, the `verify` step's `offer_rule_exists` and `rule_not_invented` checks reject it before execution — so the result holds for any backend. Reproduce against a real model with `--seller groq`.
...
Seller actions across the corpus: `{'decline': 144, 'search_products': 31, 'clarify': 11, 'quote': 4}`
